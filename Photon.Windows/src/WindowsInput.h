...
namespace Photon
{
    ...
    static LRESULT ProcessWindowMessage(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (!D3D12Application::Current->Windows->TryGetValue(System::IntPtr(hWnd), photonWindow) || (window = (WindowsWindow^)photonWindow) == nullptr)
        {
        ...
        case WM_CLOSE:
            ...
            return 0;
        case WM_NCDESTROY:
            Application::Current->Windows->Remove(System::IntPtr(hWnd));
